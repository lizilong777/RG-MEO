...
class PyTorchTrainDataset(Dataset):
    ...
    def __init__(self, head, tail, rel, ent_total, rel_total, sampling_mode='normal', bern_flag=False, filter_flag=True,
        ...
        # self.bern_flag 控制着是否使用伯努利分布来采样负样本。
        # self.filter_flag 控制着是否过滤掉一些无意义的负样本。在训练过程中，可能会生成一些负样本，它们与正样本类似，但是没有意义。
    ...
    def collate_fn(self, data):
        ...
        if self.sampling_mode == "normal":
            ...
            # batch_h：(batch_size, 1) batch_size=data中三元组的数量
